 ...
 *     | { success, performance[] }
 *
 * Faithful to index.php's filters:
 *   memos       -> addressed to me OR to 'all', not deleted
...
class StaffRecordsRepository
{
    ...
    /** Memos to me or to 'all', not deleted, newest first. */
    public function memos($userId)
    {
        $sql = "SELECT staff_memo_id, staff_memo_ref, staff_memo_date,
                       staff_memo_to_staff_id, staff_memo_from, staff_memo_subject
                FROM staff_memo
     ...
     * The visibility clause is deliberately identical to memos() above: if the
     * two ever diverge, a memo becomes openable that the list never showed.
     */
    public function memoById($memoId, $userId)
    {
        $sql = "SELECT staff_memo_id, staff_memo_ref, staff_memo_date,
...
class StaffRecordsService
{
    ...
    /** Turn a raw *_staff_id (may be 'all', a single id, or a comma-set) into a label. */
    private function resolveTargetName($raw, array $names)
    {
        $raw = trim((string)$raw);
        if ($raw === '' ) {
            return '';
        ...
        $out = array();
        foreach ($parts as $p) {
            $out[] = isset($names[(int)$p]) ? $names[(int)$p] : $p;
        }
        return implode(', ', $out);
    }

    /** Collect the numeric ids referenced across rows (skipping 'all'/blank). */
    private function collectIds(array $rows, $field)
    {
        $ids = array();
        foreach ($rows as $r) {
            foreach (explode(',', (string)$r[$field]) as $tok) {
                $tok = trim($tok);
                if ($tok !== '' && strtolower($tok) !== 'all') {
                    $ids[] = (int)$tok;
                }
            }
        }
    ...
    public function memos($person, $comp)
    {
        ...
        $ids   = $this->collectIds($rows, 'staff_memo_to_staff_id');
        $ids[] = $userId;
        ...
        foreach ($rows as $r) {
            $out[] = array(
                ...
                'to_name' => $this->resolveTargetName($r['staff_memo_to_staff_id'], $names),
                'from'    => $r['staff_memo_from'],
    ...
    public function memoDetail($person, $comp, $memoId)
    {
        ...
        $row = $this->repo->memoById($memoId, $userId);
        if ($row === null) {
        ...
        $ids = $this->collectIds(array($row), 'staff_memo_to_staff_id');
        foreach ($this->collectIds(array($row), 'staff_memo_cc_staff_id') as $ccId) {
            $ids[] = $ccId;
        }
        $names = $this->repo->namesByIds($ids);
        ...
        return array(
            ...
            'to_name'     => $this->resolveTargetName($row['staff_memo_to_staff_id'], $names),
            'cc_name'     => $this->resolveTargetName($row['staff_memo_cc_staff_id'], $names),
            'from'        => $row['staff_memo_from'],
    ...
    public function performance($person, $comp, $year)
    {
        ...
        $names = $this->repo->namesByIds($this->collectIds($rows, 'staff_performance_staff_id'));
        ...
        foreach ($rows as $r) {
            $out[] = array(
                ...
                'staff_name'    => $this->resolveTargetName($r['staff_performance_staff_id'], $names),
